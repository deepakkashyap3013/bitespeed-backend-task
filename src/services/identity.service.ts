import db from '../db';
import { Contact, IdentifyRequest, IdentifyResponse } from '../types/identity.types';
...
export class IdentityService {
    async identify(payload: IdentifyRequest): Promise<IdentifyResponse> {
        ...
        try {
            ...
            const { rows: matchingContacts }: { rows: Contact[] } = await client.query(queryText, queryParams);
            ...
            if (matchingContacts.length === 0) {
                ...
                RETURNING *;
                `;
                const { rows: [newContact] }: { rows: Contact[] } = await client.query(insertQuery, [email, phoneStr]);
            ...
            if (linkedIdsToFetch.size > 0) {
                const { rows: linkedPrimaryContacts }: { rows: Contact[] } = await client.query(
                    `SELECT * FROM "contacts" WHERE id = ANY($1::int[]) AND "link_precedence" = 'primary' AND "deleted_at" IS NULL`,
                    [Array.from(linkedIdsToFetch)]
                );
                primaryContacts.push(...linkedPrimaryContacts);
            }

            // Deduplicate and sort primary contacts by creation date
            const uniquePrimaryContactsMap = new Map<number, Contact>();
            ...
            let sortedPrimaryContacts = Array.from(uniquePrimaryContactsMap.values()).sort(
            ...
            for (let i = 1; i < sortedPrimaryContacts.length; i++) {
                ...
                if (secondaryPrimary.id !== ultimatePrimaryContact.id) {
                    ...
                    await client.query(
                        `UPDATE "contacts" SET "linked_id" = $1, "link_precedence" = 'secondary' WHERE id = $2`,
                        [ultimatePrimaryContact.id, secondaryPrimary.id]
                    );
                    // Update any contacts that were secondary to this now-demoted primary
                    await client.query(
                        `UPDATE "contacts" SET "linked_id" = $1 WHERE "linked_id" = $2`,
                        [ultimatePrimaryContact.id, secondaryPrimary.id]
                    );
                }
            }
            ...
            const { rows: allConsolidatedContacts }: { rows: Contact[] } = await client.query(
                `SELECT * FROM "contacts" WHERE (id = $1 OR "linked_id" = $1) AND "deleted_at" IS NULL`,
            ...
            if ((email || phoneStr) && (isNewEmail || isNewPhoneNumber)) {
                ...
                const exactPairExists = allConsolidatedContacts.some(
                    c => c.email === email && c.phone_number === phoneStr
                );
                if (!exactPairExists) {
                    await client.query(
                        `INSERT INTO "contacts" (email, "phone_number", "linked_id", "link_precedence", "created_at", "updated_at")
               VALUES ($1, $2, $3, 'secondary', NOW(), NOW())`,
                        [email, phoneStr, ultimatePrimaryContact.id]
                    );
                }
            }
            ...
            const { rows: finalPrimaryContactRow }: { rows: Contact[] } = await client.query(
            ...
            const { rows: secondaryContacts }: { rows: Contact[] } = await client.query(
                `SELECT * FROM "contacts" WHERE "linked_id" = $1 AND "link_precedence" = 'secondary' AND "deleted_at" IS NULL ORDER BY "created_at" ASC`,
                [finalPrimaryContact.id]
            );

            const emailsSet = new Set<string>();
            if (finalPrimaryContact.email) emailsSet.add(finalPrimaryContact.email);
            secondaryContacts.forEach(c => c.email && emailsSet.add(c.email));

            const phoneNumbersSet = new Set<string>();
            if (finalPrimaryContact.phone_number) phoneNumbersSet.add(finalPrimaryContact.phone_number);
            secondaryContacts.forEach(c => c.phone_number && phoneNumbersSet.add(c.phone_number));

            const responseEmails = [];
            if (finalPrimaryContact.email) responseEmails.push(finalPrimaryContact.email);
            emailsSet.forEach(e => {
                if (e !== finalPrimaryContact.email) responseEmails.push(e);
            });

            const responsePhoneNumbers = [];
            if (finalPrimaryContact.phone_number) responsePhoneNumbers.push(finalPrimaryContact.phone_number);
            phoneNumbersSet.forEach(p => {
                if (p !== finalPrimaryContact.phone_number) responsePhoneNumbers.push(p);
            });
